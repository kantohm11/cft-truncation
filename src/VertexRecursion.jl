...
The cross-shape (4-arm) recursion lives in `VertexRecursionCross.jl`,
and the post-recursion analysis utilities (modified vertex, propagator
factor, h-cut projection, contracted-norm helpers) in
`VertexProjections.jl`. The dispatcher `compute_vertex(...; shape)`
defined below routes to either branch.
"""
